...
/**
 * {@link ViewPagerFolio} implementation which displays {@link ViewPager} page state using a set of lines.
 */
public class LineFolio extends View implements ViewPagerFolio {
    ...
    public LineFolio(Context context) {
        super(context);
        init();
    }

    public LineFolio(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();

        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.LineFolio);
        readArray(arr);
        arr.recycle();
    }

    public LineFolio(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();

        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.LineFolio, defStyleAttr, 0);
        readArray(arr);
        arr.recycle();
    }
    ...
    private void init() {
        ...
        setLineThickness(ViewUtils.dipsToPixels(DEFAULT_THICKNESS_DIPS, metrics));
        setLineGap((int) ViewUtils.dipsToPixels(DEFAULT_LINE_GAP_DIPS, metrics));
        setMaxLineWidth((int) ViewUtils.dipsToPixels(DEFAULT_MAX_LINE_WIDTH_DIPS, metrics));
    }

    private void readArray(TypedArray arr) {
        setSelectedColor(arr.getColor(R.styleable.LineFolio_lineFolioSelectedColor, getSelectedColor()));
        setUnselectedColor(arr.getColor(R.styleable.LineFolio_lineFolioUnselectedColor, getUnselectedColor()));

        setLineThickness(arr.getDimension(R.styleable.LineFolio_lineFolioLineThickness, getLineThickness()));
        setLineGap(arr.getDimensionPixelSize(R.styleable.LineFolio_lineFolioLineGap, getLineGap()));
        setMaxLineWidth(arr.getDimensionPixelSize(R.styleable.LineFolio_lineFolioMaxLineWidth, getMaxLineWidth()));
    }
